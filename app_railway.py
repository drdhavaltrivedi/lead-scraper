...
@app.route('/api/scrape-status/<path:session_id>', methods=['GET'])
def scrape_status(session_id):
    ...
    if session_id not in scraping_sessions:
        ...
        return jsonify({
            ...
            'available_sessions': list(scraping_sessions.keys())
        }), 404
...
def not_found(e):
    """Handle 404 errors and provide debugging info"""
    path = request.path
    method = request.method
    print(f"⚠️ 404 Error: {method} {path}")
    ...
    if path.startswith('/api/'):
        return jsonify({
            ...
            'path': path,
            'method': method,
            'message': 'The requested API endpoint does not exist. Check the route path and method.'
        }), 404
